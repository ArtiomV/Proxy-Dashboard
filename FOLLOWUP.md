...
## Soft-delete ≠ unbind (продуктовый гэп, найден 2026-07)

Soft-delete модема (deleted=1) скрывает его из дашборда, но НЕ отвязывает
порт в ProxySmart: клиент продолжает платить за «удалённый» модем (кейс:
RO2_49 и RO2_56_2, удалены 23.06, продолжали тарифицироваться у БА 3 недели
→ расхождение «32 на странице модемов vs 30 в ростере»). Восстановлены
вручную (deleted=0). Правильное решение — при удалении модема предлагать
отвязку порта в ProxySmart (или явное предупреждение «биллинг продолжится»),
либо удаление должно делать и то, и другое одним действием.
