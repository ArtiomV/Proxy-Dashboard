...
(function () {
  ...
  var P = {
    ...
    // actions
    // Шестерёнка: симметричный 8-зубый вариант (стиль Tabler) — прежний путь
    // был сдвинут: контур центрирован по x=12.9 при отверстии cx=12, иконка
    // «плыла» при вращении в круглой кнопке. Здесь центр контура = (12,12).
    gear: '<circle cx="12" cy="12" r="3.2"/><path d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 0 0 2.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 0 0 1.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 0 0-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 0 0-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 0 0-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 0 0-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 0 0 1.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"/>',
  ...
  // Герб Молдовы (стилизованный): орёл со щитом на груди. Без него флаг MD
  // неотличим от RO — тот же вертикальный триколор, разница оттенков в
  // миниатюре не читается. Рисуется в 24×24 (центр 12,12) и масштабируется
  // на середину жёлтой полосы. Силуэт упрощён до читаемого пятна: крылья,
  // голова с клювом, тело-хвост, красный щит с жёлтой каймой и рогами тура.
  var MD_CREST =
    '<g fill="#4A2C14">'
    + '<path d="M11 9C8 6.5 5 5.8 2.6 6.4 4.6 7 6.2 8 7.2 9.4 5.6 9.4 4.2 9.9 3.2 10.9 5 11.2 6.6 11.1 8 10.5 7.2 11.9 6.8 13.4 6.9 14.9 8.4 13.9 9.6 12.7 10.4 11.2L11 10.6z"/>'
    + '<path d="M13 9c3-2.5 6-3.2 8.4-2.6-2 .6-3.6 1.6-4.6 3 1.6 0 3 .5 4 1.5-1.8.3-3.4.2-4.8-.4.8 1.4 1.2 2.9 1.1 4.4-1.5-1-2.7-2.2-3.5-3.7L13 10.6z"/>'
    + '<circle cx="12" cy="6.3" r="1.6"/>'
    + '<path d="M13.4 6l1.8.6-1.8.6z"/>'
    + '<path d="M10.3 10.8h3.4l-.5 6.4c0 .9-.6 1.5-1.2 1.5s-1.2-.6-1.2-1.5l-.5-6.4z"/>'
    + '</g>'
    + '<path d="M9.8 11.6h4.4v3.3c0 1.7-1.2 2.8-2.2 3.3-1-.5-2.2-1.6-2.2-3.3v-3.3z" fill="#C1272D" stroke="#FFD100" stroke-width="0.5"/>'
    + '<path d="M10.6 12.7c.8.8 2 .8 2.8 0 .3-.3.5-.7.5-1.1-.3.5-.8.8-1.4.9h-1c-.6-.1-1.1-.4-1.4-.9 0 .4.2.8.5 1.1z" fill="#FFD100"/>';
  function flagIcon(code, size) {
    ...
    var crest = '';
    if (c === 'MD') {
      // герб по центру флага, высотой ~70% полосы
      crest = '<g transform="translate(' + (w / 2).toFixed(2) + ' ' + (s / 2).toFixed(2) + ') scale(' + (s * 0.7 / 24).toFixed(3) + ') translate(-12 -12)">' + MD_CREST + '</g>';
    }
    return '<svg width="' + w + '" height="' + s + '" viewBox="0 0 ' + w + ' ' + s + '" style="vertical-align:-2px;border-radius:2px;overflow:hidden;flex-shrink:0">' + stripes + crest + '</svg>';
  }
