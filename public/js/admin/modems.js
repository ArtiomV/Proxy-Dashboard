...
case'rateNow':{var _rt=(currentData.modemRate||{})[modem.server+'_'+modem.nick];if(_rt){var _tot=_rt.rate_in_mbps+_rt.rate_out_mbps;var _rc=_tot>=20?'var(--danger)':(_tot>=5?'var(--warning)':'var(--text-1)');h+='<span style="color:'+_rc+';font-family:var(--font-mono);font-size:11px" title="Текущая скорость за окно 10 мин (замер '+(_rt.ts||'').slice(11,19)+' UTC)">↓'+_rt.rate_in_mbps+' ↑'+_rt.rate_out_mbps+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'ping':{var _pg=(currentData.modemPing||{})[modem.server+'_'+modem.nick];if(_pg){var _pgC=!_pg.ok?'var(--danger)':(!_pg.fresh?'var(--text-3)':(_pg.loss_pct>=30||_pg.latency_ms>800?'var(--warning)':'var(--success)'));h+='<span style="color:'+_pgC+';font-family:var(--font-mono);font-size:11px" title="Пинг через модем (замер бокса, ~1/мин): '+_pg.latency_ms+' мс, потери '+_pg.loss_pct+'%'+(_pg.fresh?'':' · данные протухли')+(_pg.ok?'':' · ИНТЕРНЕТА НЕТ')+'">'+_pg.latency_ms+' мс'+(_pg.loss_pct>0?' · '+_pg.loss_pct+'%':'')+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'conns':{var _ct=0,_cparts=[];modem.ports.forEach(function(p){var cs=p.conns_stats;if(!cs)return;var t=Number(cs.total)||0;_ct+=t;if(t>0)_cparts.push((p.portName||p.portID||'порт')+': '+t+' (http '+(Number(cs.http)||0)+' · socks '+(Number(cs.socks5)||0)+')');});var _con='openDetailAtTab(\''+esc(modem.nick).replace(/'/g,"\\'")+'\',\''+modem.server+'\',\'settings\')';var _cc=_ct>0?_connColor(_ct):'var(--text-3)';
...
function saveModemSettingsNew(imei,srv,btn){var rotSel=document.getElementById('msAutoRot');var rotRaw=rotSel?rotSel.value:'';var payload={serverName:srv,IMEI:imei,name:document.getElementById('msNick').value,PHONE_NUMBER:document.getElementById('msPhone').value,TARGET_MODE:document.getElementById('msMode').value,NOTES:document.getElementById('msNotes').value};if(rotRaw!=='__u')payload.AUTO_IP_ROTATION=String(parseInt(rotRaw)||0);if(btn){btn.disabled=true;btn.textContent='Сохраняю…';}showToast('Сохраняю настройки на сервере — это может занять до ~20 сек (проверка после записи)…','info',20000);var _done=function(){if(btn){btn.disabled=false;btn.textContent='Сохранить';}};api(API+'/api/admin/store_modem',{method:'POST',json:payload}).then(function(r){if(r.ok){showToast('Сохранено, применяю...','success');api(API+'/api/admin/apply_modem',{method:'POST',json:{imei:imei,serverName:srv}}).then(function(r2){_done();r2.ok?showToast('Применено','success'):showToast('Сохранено, но не применено: '+(r2.error||''),'warning');setTimeout(loadData,3000)}).catch(function(e){_done();showToast('Сохранено, но не применено: '+esc(e.message),'warning');setTimeout(loadData,3000)})}else{_done();showToast((r.error||'Ошибка')+(r.details?': '+esc(r.details):''),'error')}}).catch(function(e){_done();showToast('Ошибка: '+esc(e.message),'error')})}
