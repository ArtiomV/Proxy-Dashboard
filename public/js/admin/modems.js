...
// (heatmap and aggregate statistics), or null if included.
...
function _excludeReason(modem){
  ...
  if(!anyBound){
    return {
      type:'unbound',
      label:'Без клиента',
      tooltip:'Исключён из клиентской статистики: нет привязки к клиенту (ни один порт не имеет portName).'
    };
  }
  return null;
}
...
// ========== MODEM TABLE ==========
function renderServerFilter(){var sv=currentData.servers||[];var ls='font-size:11px;display:flex;align-items:center;gap:6px;padding:3px 8px;cursor:pointer;border-radius:3px';var h='<label style="'+ls+'"><input type="radio" name="srvF" '+(activeServerFilter==='all'?'checked':'')+' data-on-change="setServerFilter(\'all\')"> Все серверы</label>';
  // Group by country
  ...
  Object.keys(seen).forEach(function(cc){var g=seen[cc];var ccActive=g.servers.indexOf(activeServerFilter)!==-1;
    h+='<label style="'+ls+';font-weight:600;font-size:10px;padding-top:6px"><input type="radio" name="srvF" '+(activeServerFilter==='country:'+cc?'checked':'')+' data-on-change="setServerFilter(\'country:'+cc+'\')"> '+g.flag+' '+g.name+' <span style="color:var(--text-3);font-weight:400">(все)</span></label>';
    g.servers.sort().forEach(function(sn){h+='<label style="'+ls+';padding-left:20px"><input type="radio" name="srvF" '+(activeServerFilter===sn?'checked':'')+' data-on-change="setServerFilter(\''+sn+'\')"> '+sn+'</label>'})});
...
function renderClientFilterDD(){var el=document.getElementById('clientFilterDD');if(!el||!currentData)return;var cls=currentData.clients||[];var h='<label style="display:flex;align-items:center;gap:6px;padding:3px 8px;cursor:pointer;font-size:11px;border-radius:3px"><input type="radio" name="clF" '+(activeClientFilter===''?'checked':'')+' data-on-change="setClientFilter(\'\')"> Все клиенты</label>';cls.filter(function(c){return c.portName&&c.modemCount>0}).forEach(function(c){h+='<label style="display:flex;align-items:center;gap:6px;padding:3px 8px;cursor:pointer;font-size:11px;border-radius:3px"><input type="radio" name="clF" '+(activeClientFilter===c.portName?'checked':'')+' data-on-change="setClientFilter(\''+esc(c.portName)+'\')"> '+esc(c.name)+'</label>'});el.innerHTML=h}
...
function renderColSelector(){var h='';for(var i=0;i<COLUMNS.length;i++){var c=COLUMNS[i];if(c.id==='status'||c.id==='actions'||c.id==='bulk'||c.id==='rail')continue;h+='<label><input type="checkbox" '+(c.visible?'checked':'')+' data-on-change="toggleCol(\''+c.id+'\',this.checked)">'+c.label+'</label>'}document.getElementById('colDropdown').innerHTML=h}
...
// Розничный пул: порты со статусом 'blocked' не работают и живут до hold_until,
// потом их сносит retail-guard. Помечаем их красным в таблице и карточке модема.
// Кэш на 60 сек — loadData дёргает рендер каждые 10 сек, пул не должен фетчиться так же часто.
var _retailBlockedMap=null,_retailBlockedAt=0,_retailBlockedPending=false;
function _retailBlockedRefresh(){
  var now=Date.now();
  if(_retailBlockedPending||now-_retailBlockedAt<60000)return;
  _retailBlockedPending=true;
  api(API+'/api/admin/retail/pool').then(function(d){
    _retailBlockedPending=false;_retailBlockedAt=Date.now();
    var map={};
    ((d&&d.rows)||[]).forEach(function(r){
      if(r&&r.status==='blocked')map[r.server+'|'+String(r.port_id||'').replace(/^S\d+_/,'')]=r.hold_until||'';
    });
    _retailBlockedMap=map;
    renderTable(true);
    if(currentDetailModem){var t=document.querySelector('.modal-tab.active');if(t&&t.dataset.tab==='settings')renderTabContent('settings');}
  }).catch(function(){_retailBlockedPending=false;});
}
// null = не заблокирован; строка ('' или hold_until ISO) = заблокирован.
function _portBlockedInfo(server,pid){
  _retailBlockedRefresh();
  if(!_retailBlockedMap)return null;
  var k=server+'|'+String(pid||'').replace(/^S\d+_/,'');
  return _retailBlockedMap.hasOwnProperty(k)?_retailBlockedMap[k]:null;
}
// Клиент заблокирован (антифрод или долг): его порты уже не работают и уйдут
// в удаление. Пул розницы пуст для B2B, поэтому флаг берём из карточки
// клиента по portName — иначе такие порты на странице модемов не краснели.
function _clientBlockedByPortName(pn){
  if(!pn)return null;
  var cls=(typeof currentData!=='undefined'&&currentData&&currentData.clients)||[];
  for(var i=0;i<cls.length;i++){var c=cls[i];if(c.portName===pn&&(c.blocked||c.debtBlocked))return c;}
  return null;
}
function _blockedPortTitle(hold){return 'Порт заблокирован: прокси не работает'+(hold?'. Будет удалён '+_fmtIsoDt(hold):'')+' — нажмите для настроек порта';}
function _blockedClientTitle(){return 'Клиент заблокирован: прокси уже не работают — порты будут удалены, если не оплатить';}
function _httpEligibility(modem){
  if(!_hasActiveClient(modem))return{ok:false,text:'—',title:'HTTP-мониторинг не нужен: модем свободен или аренда истекла'};
  var now=Date.now(),ports=modem.ports||[];
  for(var i=0;i<ports.length;i++){
    var p=ports[i],pn=String(p.portName||'').trim(),hp=Number(p.HTTP_PORT);
    var exp=p.PROXY_VALID_BEFORE?Date.parse(p.PROXY_VALID_BEFORE):NaN;
    if(pn&&!/^randomport\d*$/i.test(pn)&&Number.isInteger(hp)&&hp>0&&hp<=65535&&String(p.LOGIN||'').trim()&&String(p.PASSWORD||'').trim()&&!/^(1|true|yes|on)$/i.test(String(p.IS_EXPIRED||''))&&!/^(1|true|yes|on)$/i.test(String(p.IS_OVER_QUOTA||''))&&(isNaN(exp)||exp>=now))return{ok:true,text:'ожидает',title:'Реквизиты валидны; результат появится после ближайшего HTTP-прогона'};
  }
  return{ok:false,text:'нет доступа',title:'HTTP-чек пропущен: нет действующего клиентского HTTP-порта с валидными логином и паролем'};
}
...
function _modemIssue(modem){
  ...
  // одному флапнувшему чеку: мобильный прокси изредка теряет один ping sample,
  // но работает (трафик идёт).
...
function _serverDownBadge(srv){var d=_serverDownInfo(srv);if(!d.down)return'';return'<span class="srv-down-badge" title="Сервер не отвечает — показаны последние данные из кеша">'+icon('alert',11)+' Сервер недоступен'+(d.ageMin?' · '+d.ageMin+' мин назад':'')+'</span>';}
function _modemFlags(m){var f='';
  if(m.isTestPool)f+='<span class="mflag" style="color:var(--text-2)" title="Тестовый пул симулятора — в счётчики парка не входит">'+icon('flask',11)+'</span>';
  if(m.httpRedirect)f+='<span class="mflag" style="color:var(--danger)" title="Оператор навязал редирект — SIM без денег / блок">'+icon('stop',11)+'</span>';
  ...
  if(m.rebootScore!=null&&Number(m.rebootScore)>=70)f+='<span class="mflag" style="color:var(--warning)" title="Нужен ребут (score '+m.rebootScore+')">'+icon('refresh',11)+'</span>';
  ...
  else if(m.msg&&!_msgIsNoise(m.msg))f+='<span class="mflag" style="color:var(--warning)" title="ProxySmart: '+esc(m.msg)+'">'+icon('alert',11)+'</span>';
  return f;}
...
function tileMenu(e,tile){e.stopPropagation();var n=tile.dataset.nick,s=tile.dataset.server,i=tile.dataset.imei||'',mdl=tile.dataset.model||'';var d='data-imei="'+i+'" data-server="'+s+'" data-nick="'+esc(n)+'"';var m=document.getElementById('tileMenu');if(!m){m=document.createElement('div');m.id='tileMenu';m.className='tile-menu';document.body.appendChild(m);}
  m.innerHTML='<div class="tm-h">'+esc(n)+' <span style="color:var(--text-3);font-weight:400">'+esc(_serverDisplayLabel(s))+'</span></div>'
    +'<button class="tm-item" '+d+' data-on-click="rebootModem(this);tmClose()">'+icon('power')+' Ребут</button>'
    +(/MF289/i.test(mdl)?'':'<button class="tm-item" '+d+' data-on-click="usbReset(this);tmClose()">'+icon('off')+' USB-ресет</button>')
    +'<button class="tm-item" '+d+' data-on-click="readdModem(this);tmClose()">'+icon('refresh')+' Re-Add (переподключить)</button>'
    +'<div style="height:1px;background:var(--border);margin:4px 0"></div>'
    +'<button class="tm-item" data-on-click="tmClose();openDetailAtTab(this.dataset.nick,this.dataset.srv,\'info\')" data-nick="'+esc(n)+'" data-srv="'+s+'">'+icon('info')+' Подробнее</button>'+((i&&tile.dataset.stale==='1')?'<div style="height:1px;background:var(--border);margin:4px 0"></div><button class="tm-item" style="color:var(--danger)" data-on-click="tmClose();deleteModem(this.dataset.srv,this.dataset.pid,this.dataset.nick)" data-srv="'+s+'" data-pid="meta_'+i+'" data-nick="'+esc(n)+'">'+icon('trash')+' Удалить</button>':'');
...
function renderModemGrid(groups){var out='';COUNTRY_ORDER.forEach(function(srv){var modems=groups[srv];if(!modems||!modems.length)return;var ci=COUNTRIES[srv]||{};var online=0;modems.forEach(function(m){var s=getModemStatus(m);if(s==='online'||s==='rotating')online++});var fb=(currentData.fleet&&currentData.fleet.byServer&&currentData.fleet.byServer[srv])||null;var fOn=fb?((fb.working!=null)?fb.working:fb.online):online;var fN=fb?fb.total:modems.length;if(fN<fOn)fN=fOn;out+='<div style="font-size:12px;color:var(--text-2);margin:12px 24px 8px;display:flex;align-items:center;gap:8px;flex-wrap:wrap"><span style="font-family:var(--font-mono);font-weight:700;color:var(--accent)">'+esc(ci.displayName||srv)+'</span>'+(ci.name?'<span>· '+esc(ci.name)+'</span>':'')+(ci.address?'<span style="color:var(--text-3)">'+icon('pin',11)+' '+esc(ci.address)+'</span>':'')+'<span style="color:'+(fOn>=fN?'var(--success)':'var(--warning)')+';font-weight:600">'+fOn+'/'+fN+' в работе</span>'+_serverDownBadge(srv)+'<span style="margin-left:auto;display:inline-flex;gap:6px"><button class="srv-act" data-on-click="reconnectAll(\''+srv+'\')" title="Re-Add всех офлайн-модемов сервера">↻ Переподключить модемы</button><button class="srv-act srv-act-danger" data-on-click="rebootServer(\''+srv+'\')" title="Перезагрузить весь сервер (нужен пароль)">'+icon('power')+' Ребут сервера</button></span></div><div class="mtile-grid" style="padding:0 24px 4px">';var sorted=modems.slice().sort(function(a,b){return String(a.nick).localeCompare(String(b.nick),undefined,{numeric:true,sensitivity:'base'})});sorted.forEach(function(m){var c=_tileColor(m);var st=getModemStatus(m);var sub;var iss;if(m.webappDown){sub='<span style="color:var(--danger)">WebApp недоступен</span>';}else if(st==='rotating'){sub='<span style="color:var(--accent)">Смена IP…</span>';}else if(st==='rebooting'){sub='<span style="color:var(--accent)">Перезагрузка…</span>';}else if(st==='offline'){var off=_offlineReason(m);sub='<span style="color:var(--text-3)">'+(off?esc(off.reason):'офлайн')+'</span>';}else if(!_hasActiveClient(m)){sub='<span style="color:var(--text-3)">'+(_allClientsExpired(m)?'Оплата истекла':'Свободен')+'</span>';}else{iss=_modemIssue(m);if(iss&&iss.kind==='err')iss=null;var _pt=m.pcErrorPctToday;var _tBad=(m.pcErrToday>0&&_pt!=null&&_pt>=_errorRateThreshold);var _tWarn=(m.pcErrToday>0&&_pt!=null&&_pt>=5);if(iss&&iss.sev>=4){sub='<span style="color:var(--danger)">'+esc(iss.reason)+'</span>';}else if(_tBad||_tWarn){sub='<span style="color:'+(_tBad?'var(--danger)':'var(--warning)')+'">Сбои пинга: '+_pt+'%</span>';}else if(iss){sub='<span style="color:var(--warning)">'+esc(iss.reason)+'</span>';}else if(m.pcErrToday>0){sub='<span style="color:var(--text-3)">Сбои пинга: '+(_pt>=1?_pt+'%':'<1%')+'</span>';}else{sub='<span style="color:var(--text-3)">'+(m.pcChecksToday>0?'Без сбоев':'Ок')+'</span>';}}var _cls=_modemClients(m);var _clf=esc(_cls.join(', '));
...
out+='<div class="modem-tile" data-nick="'+esc(m.nick)+'" data-server="'+m.server+'" data-imei="'+(m.rawImei||'')+'" data-model="'+esc(m.model||'')+'" data-stale="'+(_isStaleModem(m)?'1':'')+'" data-on-click="tileMenu(event,this)" style="border-left-color:'+c.b+';'+(c.bg?'background:'+c.bg+';':'')+'"><div class="mt-nick">'+esc(m.nick)+_modemFlags(m)+'</div><div class="mt-client" title="'+_clf+'">'+(_cls.length?_clf:'<span style="color:var(--text-3)">—</span>')+'</div>'+(m.operator?'<div class="mt-op" title="Оператор"><i></i>'+esc(m.operator)+'</div>':'')+'<div class="mt-sub">'+sub+'</div>'+_cConn+'</div>';});out+='</div>';});return out||'<div style="padding:40px;text-align:center;color:var(--text-3)">Нет модемов по фильтру</div>';}
  var _mkColor={webapp:'var(--danger)',sim:'var(--danger)',proxy:'var(--danger)',offline:'var(--danger)',loss:'var(--danger)',err:'var(--warning)',reboot:'var(--warning)'};

function renderTable(auto){
  if(!currentData||!currentData._modemMap)return;
  // auto=true — авторефреш: пока пользователь что-то вводит/выбирает в таблице
  // или панели фильтров (фокус в поле), ререндер откладываем — иначе каждые
  // 60 сек стирались неприменённые значения и закрывались открытые селекты.
  if(auto){var _ae=document.activeElement;if(_ae&&(_ae.tagName==='INPUT'||_ae.tagName==='SELECT'||_ae.tagName==='TEXTAREA')){var _tw=document.getElementById('tableWrap'),_tb=document.getElementById('modemsTopBar');if((_tw&&_tw.contains(_ae))||(_tb&&_tb.contains(_ae)))return;}}
  try{renderModemsTopBar()}catch(_e){}var map=currentData._modemMap,search=(((document.getElementById('modemSearch')||document.getElementById('searchBox')||{}).value)||'').toLowerCase(),cols=COLUMNS.filter(function(c){return c.visible}),wrap=document.getElementById('tableWrap');
  var groups={};COUNTRY_ORDER.forEach(function(s){groups[s]=[]});
  var totalFiltered=0;
  for(var imei in map){var modem=map[imei],srv=modem.server;if(activeQuickFilter!=='all'){var _qst=getModemStatus(modem);if(activeQuickFilter==='online'){if(_qst!=='online'&&_qst!=='rotating')continue}else if(activeQuickFilter==='offline'){if(_qst!=='offline')continue}else if(activeQuickFilter==='problem'){if(!_modemIssue(modem)&&!_offlineReason(modem))continue}else if(activeQuickFilter==='sim'){var _qi=_modemIssue(modem);if(!_qi||_qi.kind!=='sim')continue}else if(activeQuickFilter==='idle'){if(_qst==='offline'||_modemTrafGb(modem)>0.05)continue}else if(activeQuickFilter==='free'){if(_hasActiveClient(modem))continue}}if(activeServerFilter!=='all'){if(activeServerFilter.indexOf('country:')===0){var _fc=activeServerFilter.slice(8);var _sc=(COUNTRIES[srv]||{}).country||'';if(_sc!==_fc)continue}else if(srv!==activeServerFilter)continue}var status=getModemStatus(modem);if(activeStatusFilter==='online'&&status!=='online'&&status!=='rotating')continue;if(activeStatusFilter==='offline'&&status!=='offline')continue;if(activeClientFilter){var pns=modem.ports.map(function(p){return p.portName||''});if(pns.indexOf(activeClientFilter)===-1)continue}if(search){var ss=[modem.nick,modem.extIp,modem.operator,modem.phone].join(' ').toLowerCase();modem.ports.forEach(function(p){ss+=' '+(p.portName||'')+' '+(p.LOGIN||'')});if(ss.indexOf(search)===-1)continue}if(!groups[srv])groups[srv]=[];groups[srv].push(modem);totalFiltered++}
  var totalAll=Object.keys(map).length;
  var ctr=document.getElementById('searchCounter');
  if(ctr){var hasFilter=search||activeClientFilter||activeServerFilter!=='all'||activeStatusFilter!=='all'||activeQuickFilter!=='all';ctr.style.display=hasFilter?'':'none';if(hasFilter){ctr.title='Показано '+totalFiltered+' из '+totalAll+' модемов';ctr.innerHTML='<b>'+totalFiltered+'</b><span>из '+totalAll+'</span>'}}
  ...
  cols.forEach(function(col){var w=col.width?' style="width:'+col.width+'"':'';var ar=col.sortable&&sortCol===col.id?' '+(sortDir==='asc'?'\u25B2':'\u25BC'):'';var cl=col.sortable?' data-on-click="setSort(\''+col.id+'\')"':'';html+='<th'+w+cl+'>'+col.label+ar+'</th>'});
  html+='</tr></thead><tbody>';
  COUNTRY_ORDER.forEach(function(srv){var modems=groups[srv];if(!modems||!modems.length)return;var ci=COUNTRIES[srv]||{};var online=0;modems.forEach(function(m){var s=getModemStatus(m);if(s==='online'||s==='rotating')online++});
    ...
    html+='<input type="checkbox" class="srv-bulk-chk" data-server="'+esc(srv)+'" data-on-change="bulkToggleServer(\''+esc(srv)+'\',this)" style="cursor:pointer;margin:0" title="Выбрать все модемы '+esc(_serverDisplayLabel(srv))+'">';
    html+='<span style="font-size:14px">'+(ci.flag||'')+'</span>';
    html+='<span style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:var(--accent)">'+esc(ci.displayName||srv)+'</span>';
    ...
    html+='<div style="margin-left:auto;display:flex;gap:6px"><button class="srv-act" data-on-click="resetCompleteServer(\''+srv+'\')">↻ Сброс IP</button><button class="srv-act srv-act-danger" data-on-click="rebootServer(\''+srv+'\')">'+icon('power')+' Ребут</button></div>';
    html+='</div></td></tr>';
    ...
    // threshold the backend uses to exclude stale modems from analytics
    // (see getStaleNicks in server.js). Modems that have never
    ...
    modems.forEach(function(modem){var status=getModemStatus(modem),port=modem.ports[0]||{},bw=port._bw||{},ci=COUNTRIES[modem.server]||{};var isSel=!!(window._bulkSel&&window._bulkSel[modem.rawImei]);var h='<tr class="modem-row" data-nick="'+esc(modem.nick)+'" data-server="'+modem.server+'" data-on-click="rowOpen(event,this)" style="cursor:pointer">';cols.forEach(function(col){var _dl=(col.label||'').replace(/<[^>]*>/g,'').replace(/"/g,'').trim();h+='<td class="cell-'+col.id+'" data-label="'+_dl+'"'+(col.id==='rail'?' style="padding:0;text-align:center"':'')+'>';switch(col.id){case'rail':{var _ar=_attnReason(modem);var _rc=_ar?((_ar.kind==='offline'&&_ar.stale)?'var(--text-3)':(_mkColor[_ar.kind]||'var(--warning)')):null;h+=_rc?'<span class="mk-row-rail" style="background:'+_rc+'"></span>':'';break;}case'bulk':h+='<input type="checkbox" class="bulk-chk" data-imei="'+modem.rawImei+'" data-server="'+modem.server+'" data-nick="'+esc(modem.nick)+'" '+(isSel?'checked':'')+' data-on-change="updateBulkPanel()" style="cursor:pointer;margin:0">';break;case'status':h+=_statusPill(status,modem);break;case'nick':h+='<strong>'+esc(modem.nick)+'</strong>'+_excludeChip(modem)+_modemFlags(modem);break;case'server':h+='<span style="font-size:10px">'+(ci.flag||'')+' '+esc(_serverDisplayLabel(modem.server))+'</span>';break;case'portName':{var un=[];modem.ports.forEach(function(p){var n=(p.portName||'').trim();if(n&&!un.some(function(x){return x.n===n}))un.push({n:n,pid:p.portID||''});});var _on='openDetailAtTab(\''+esc(modem.nick)+'\',\''+modem.server+'\',\'settings\')';if(!un.length){h+='<button class="btn btn-sm" style="font-size:10px;padding:2px 7px" data-on-click="'+_on+'">+ Порт</button>'}else{h+=un.slice(0,2).map(function(pp){var _bh=_portBlockedInfo(modem.server,pp.pid);var _bc=_clientBlockedByPortName(pp.n);var _blk=_bh!==null||!!_bc;return'<span class="port-badge" style="'+(_blk?'background:rgba(232,65,65,.12);color:var(--danger);border:1px solid var(--danger)':'background:var(--bg-3);color:var(--accent);border:1px solid var(--border)')+';padding:1px 8px;border-radius:20px;font-size:10px;cursor:pointer" data-on-click="'+_on+'" title="'+(_bh!==null?esc(_blockedPortTitle(_bh)):_blk?esc(_blockedClientTitle()):'Настройки порта')+'">'+esc(pp.n)+'</span>'}).join(' ');if(un.length>2)h+=' <span class="port-badge" title="'+esc(un.slice(2).map(function(x){return x.n}).join(', '))+'" style="background:var(--accent-dim);color:var(--accent);padding:1px 6px;border-radius:20px;font-size:10px;cursor:pointer" data-on-click="'+_on+'">+'+(un.length-2)+'</span>';h+=' <button class="btn btn-sm" style="font-size:10px;padding:2px 7px;opacity:0.5" data-on-click="'+_on+'" title="Добавить порт">+</button>'}break;}case'creds':{var sip=ci.serverIp||'';var _hasPort=false;modem.ports.forEach(function(p){if(!p.HTTP_PORT&&!p.SOCKS_PORT)return;_hasPort=true;var _pport=p.HTTP_PORT||p.SOCKS_PORT;var _auth=p.LOGIN?':'+esc(p.LOGIN)+':'+esc(p.PASSWORD||''):'';if(modem.ports.length>1&&(p.portName||p.portID))h+='<div style="font-size:9px;color:var(--accent);font-weight:600;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(p.portName||p.portID||'')+'</div>';h+='<div style="display:flex;align-items:center;gap:4px"><span class="mono" style="font-size:10px">'+sip+':'+_pport+'</span><button class="copy-btn" title="Копировать ip:port:login:pass" data-on-click="copyText(\''+sip+':'+_pport+_auth+'\',this)">\u{1F4CB}</button></div>'});if(!_hasPort)h+='-';break;}case'loginpass':{var _hasLogin=false;modem.ports.forEach(function(p){if(!p.LOGIN)return;_hasLogin=true;if(modem.ports.length>1&&(p.portName||p.portID))h+='<div style="font-size:9px;color:var(--accent);font-weight:600;margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(p.portName||p.portID||'')+'</div>';h+='<div><span class="mono">'+esc(p.LOGIN)+':••••</span> <button class="copy-btn" data-on-click="copyText(\''+esc(p.LOGIN)+':'+esc(p.PASSWORD||'')+'\',this)">\u{1F4CB}</button></div>'});if(!_hasLogin)h+='-';break;}case'extIp':if(modem.extIp==='IP_RESET'||modem.isRotating){h+='<span class="mono" style="color:var(--warning)">Ротация</span>'}else if(modem.extIp){h+='<span class="mono'+(modem.ipStuck?' ip-stuck':'')+'">'+modem.extIp+(modem.ipStuck?'<span class="ip-stuck-badge" title="IP не менялся '+modem.ipSinceHours+'ч"> '+modem.ipSinceHours+'ч</span>':'')+'</span>'}else{h+='-'}break;case'netType':{var _op=modem.operator?esc(modem.operator):'<span style="color:var(--text-3)">—</span>';var _nt=(modem.netType||'').toString().toUpperCase();var _ntc=/3G|HSPA|UMTS/.test(_nt)?'var(--warning)':(/2G|GPRS|EDGE/.test(_nt)?'var(--danger)':'var(--text-3)');var _nth=_nt?'<span style="font-size:9px;font-weight:600;color:'+_ntc+'">'+esc(_nt)+'</span>':'';h+='<div style="display:flex;align-items:center;gap:7px"><span style="font-size:12px;color:var(--text-1)">'+_op+'</span>'+_nth+renderSignalBars(modem.signal)+'</div>';break;}case'signal':h+=renderSignalBars(modem.signal);break;case'operator':h+=esc(modem.operator)||'-';break;case'phone':h+=modem.phone?'<span class="mono">'+esc(modem.phone)+'</span>':'-';break;case'trafficDay':{var _dinSum=0,_doutSum=0;modem.ports.forEach(function(p){var _pb=p._bw||{};_dinSum+=parseTraffic(_pb.bandwidth_bytes_day_in);_doutSum+=parseTraffic(_pb.bandwidth_bytes_day_out)});h+='<span class="mono">'+fmtGb(_dinSum+_doutSum)+'</span>';break;}case'trafficMon':{var _minSum=0,_moutSum=0;modem.ports.forEach(function(p){var _pb=p._bw||{};_minSum+=parseTraffic(_pb.bandwidth_bytes_month_in);_moutSum+=parseTraffic(_pb.bandwidth_bytes_month_out)});h+='<span class="mono">'+fmtGb(_minSum+_moutSum)+'</span>';break;}case'speed':{var _dl=Number(modem.lastSpeedDl||0),_ul=Number(modem.lastSpeedUl||0);if(_dl||_ul){var _isLow=_dl<_minSpeedThreshold||_ul<_minSpeedThreshold;var _spDateMs=modem.lastSpeedDate?Date.parse(modem.lastSpeedDate):0;var _ageH=_spDateMs?Math.floor((Date.now()-_spDateMs)/3600000):null;var _STALE_H=48;if(_ageH!==null&&_ageH>_STALE_H){var _ageLbl=_ageH<48?_ageH+'\u0447 \u043D\u0430\u0437\u0430\u0434':Math.floor(_ageH/24)+'\u0434 \u043D\u0430\u0437\u0430\u0434';h+='<span class="speed-cell" title="\u041F\u043E\u0441\u043B\u0435\u0434\u043D\u0438\u0439 \u0437\u0430\u043C\u0435\u0440: '+(_ageLbl)+' \u00B7 '+esc(modem.lastSpeedDate||'')+'\\n\u2193'+_dl.toFixed(1)+' / \u2191'+_ul.toFixed(1)+' Mbps" style="color:var(--text-3);font-size:11px;cursor:help">\u2014 <span style="font-size:9px">('+_ageLbl+')</span></span>'}else{h+='<span class="speed-cell" title="\u0417\u0430\u043C\u0435\u0440: '+esc(modem.lastSpeedDate||'')+'" style="'+(_isLow?'background:rgba(255,56,60,.15);border-radius:4px;padding:2px 4px':'')+'"><span class="speed-dl">\u2193'+_dl.toFixed(1)+'</span> / <span class="speed-ul">\u2191'+_ul.toFixed(1)+'</span>'+(_isLow?' \u26A0':'')+'</span>'}}else{h+='-'}break;}case'uptime':if(modem.uptimePct!==undefined){var upCls=parseFloat(modem.uptimePct)>=99?'good':parseFloat(modem.uptimePct)>=95?'warn':'bad';h+='<span class="uptime-pct '+upCls+'">'+modem.uptimePct+'%</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;
case'rateNow':{var _rt=(currentData.modemRate||{})[modem.server+'_'+modem.nick];if(_rt){var _tot=_rt.rate_in_mbps+_rt.rate_out_mbps;var _rc=_tot>=20?'var(--danger)':(_tot>=5?'var(--warning)':'var(--text-1)');h+='<span style="color:'+_rc+';font-family:var(--font-mono);font-size:11px" title="Текущая скорость за окно 10 мин (замер '+(_rt.ts||'').slice(11,19)+' UTC)">↓'+_rt.rate_in_mbps+' ↑'+_rt.rate_out_mbps+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'ping':{var _pg=(currentData.modemPing||{})[modem.server+'_'+modem.nick];if(_pg){var _pgC=!_pg.ok?'var(--danger)':(!_pg.fresh?'var(--text-3)':(_pg.loss_pct>=30||_pg.latency_ms>800?'var(--warning)':'var(--success)'));h+='<span style="color:'+_pgC+';font-family:var(--font-mono);font-size:11px" title="Пинг через модем (замер ProxySmart, ~1/мин): '+_pg.latency_ms+' мс, потери '+_pg.loss_pct+'%'+(_pg.fresh?'':' · данные протухли')+(_pg.ok?'':' · ИНТЕРНЕТА НЕТ')+'">'+_pg.latency_ms+' мс'+(_pg.loss_pct>0?' · '+_pg.loss_pct+'%':'')+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'http':{var _hc=(currentData.modemHttpCheck||{})[modem.server+'_'+modem.nick];if(_hc){var _hcAge=_hc.ts?Date.now()-Date.parse(_hc.ts):0;var _hcStale=_hcAge>30*60*1000;var _hcSkip=_hc.error==='offline'||_hc.error==='no_valid_client_credentials';var _hcBad=!_hcSkip&&(!!_hc.error||(_hc.status!=null&&(_hc.status<200||_hc.status>=400))||_hc.content_ok===0);var _hcColor=(_hcStale||_hcSkip)?'var(--text-3)':(_hcBad?'var(--danger)':'var(--success)');var _hcText=_hcSkip?(_hc.error==='offline'?'офлайн':'нет доступа'):_hcBad?'ошибка':(_hc.total_ms!=null?_hc.total_ms+' мс':String(_hc.status||'OK'));var _hcTitle=_hcSkip?(_hc.error==='offline'?'HTTP-чек пропущен: модем офлайн':'HTTP-чек пропущен: нет действующего клиентского HTTP-порта'):'HTTP '+(_hc.status||'—')+(_hc.total_ms!=null?' · '+_hc.total_ms+' мс':'')+(_hc.error?' · '+_hc.error:'')+(_hcStale?' · данные старше 30 минут':'');h+='<span style="color:'+_hcColor+';font-family:var(--font-mono);font-size:11px" title="'+esc(_hcTitle)+'">'+esc(_hcText)+'</span>'}else{var _he=_httpEligibility(modem);h+='<span style="color:var(--text-3);font-size:10px;cursor:help" title="'+esc(_he.title)+'">'+esc(_he.text)+'</span>'}break;}case'conns':{var _ct=0,_cparts=[];modem.ports.forEach(function(p){var cs=p.conns_stats;if(!cs)return;var t=Number(cs.total)||0;_ct+=t;if(t>0)_cparts.push((p.portName||p.portID||'порт')+': '+t+' (http '+(Number(cs.http)||0)+' · socks '+(Number(cs.socks5)||0)+')');});var _con='openDetailAtTab(\''+esc(modem.nick).replace(/'/g,"\\'")+'\',\''+modem.server+'\',\'settings\')';var _cc=_ct>0?_connColor(_ct):'var(--text-3)';
...
h+='<span data-on-click="event.stopPropagation();'+_con+'" style="cursor:pointer;display:inline-flex;align-items:center;gap:6px" title="'+esc(_cparts.join('\n')||'нет активных подключений')+(_spark?'&#10;за час: макс '+_hmax:'')+'&#10;клик — лимиты порта">'+_spark+'<span class="mono" style="font-weight:'+(_ct>0?'600':'400')+';color:'+_cc+'">'+_ct+'</span></span>';break;}case'errors':{var ep=modem.pcErrorPct;if(ep!==null&&ep!==undefined){var eCls=ep>=_errorRateThreshold?'pc-bad':ep>0?'pc-warn':'pc-good';h+='<span class="pc-err '+eCls+'">'+ep+'%</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'rotation':{var _rr=modem.autoRotation;if(_rr===''||_rr==null){h+='<span style="color:var(--text-3);font-size:10px" title="Нет данных с сервера">?</span>'}else{var rotMin=parseInt(_rr)||0;if(rotMin>0){var rotStr=rotMin>=60?(rotMin/60).toFixed(0)+'ч':rotMin+'м';h+='<span class="mono" style="font-size:11px">'+rotStr+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">Выкл</span>'}}break;}case'band':h+=modem.band?'<span class="mono" style="font-size:11px">'+esc(modem.band)+'</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'simStatus':{var _ss=(modem.simStatus||'');if(!_ss||_ss==='UNKNOWN'){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _ok=/\bOK\b|READY/.test(_ss);h+='<span class="pc-err '+(_ok?'pc-good':'pc-bad')+'" style="font-size:10px" title="'+esc(_ss)+'">'+esc(_ss)+'</span>'}break;}case'rebootScore':{var _rs=modem.rebootScore;if(_rs==null){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _rc=_rs>=70?'pc-bad':_rs>=50?'pc-warn':'pc-good';h+='<span class="pc-err '+_rc+'" style="font-size:10px">'+_rs+'</span>'}break;}case'httpRedirect':h+=modem.httpRedirect?'<span class="pc-err pc-bad" style="font-size:10px" title="Оператор навязал редирект — SIM без денег/блок">редирект</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'isLocked':h+=modem.isLocked?'<span title="Модем занят операцией/ротацией">'+icon('lock',11)+'</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'actions':{var d='data-imei="'+modem.rawImei+'" data-server="'+modem.server+'" data-nick="'+esc(modem.nick)+'"';h+='<div class="actions-cell" style="display:flex;gap:4px;align-items:center;justify-content:flex-end">'+(status!=='offline'?'<button class="row-act" '+d+' title="Сбросить IP" data-on-click="resetIp(this)">↻</button><button class="row-act" '+d+' title="Ребут модема" data-on-click="rebootModem(this)">'+icon('power',12)+'</button>':'')+'<button class="btn-details" '+d+' data-on-click="showDetails(this)"><svg width="12" height="12" viewBox="0 0 13 13" fill="none"><rect x="1" y="1" width="11" height="11" rx="2" stroke="currentColor" stroke-width="1.1"/><line x1="4" y1="4.5" x2="9" y2="4.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="6.5" x2="9" y2="6.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="8.5" x2="7" y2="8.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/></svg>Инфо</button>';
...
if(_pid){h+='<button class="btn-delete-modem" title="Удалить модем из дашборда" data-on-click="deleteModem(\''+modem.server+'\',\''+_pid+'\',\''+esc(modem.nick)+'\')">'+icon('trash',12)+'</button>';}}
...
function rebootServer(srv){var label=_serverDisplayLabel(srv);uiPrompt('Перезагрузить сервер '+label+'?\nВсе модемы будут недоступны на время перезагрузки.\n\nВведите пароль для подтверждения:',{title:'Ребут сервера '+label,password:true,okText:'Перезагрузить',danger:true}).then(function(pwd){if(!pwd)return;showToast('Перезагрузка сервера '+label+'…','info');api(API+'/api/admin/reboot_server',{method:'POST',json:{serverName:srv,password:pwd}}).then(function(r){r.ok?showToast('Сервер '+label+' перезагружается','success'):showToast(r.error||'Ошибка','error')}).catch(function(){showToast('Ошибка сети','error')})})}
function resetCompleteServer(srv){var label=_serverDisplayLabel(srv);uiPrompt('Сбросить IP на ВСЕХ модемах сервера '+label+'?\n\nВведите пароль для подтверждения:',{title:'Сброс IP · '+label,password:true,okText:'Сбросить',danger:true}).then(function(pwd){if(!pwd)return;showToast('Сброс IP всех модемов '+label+'...','info');api(API+'/api/admin/reset_complete',{method:'POST',json:{serverName:srv,password:pwd}}).then(function(r){r.ok?showToast('Сброшено '+r.reset+'/'+r.total+' модемов','success'):showToast(r.error||'Ошибка','error');setTimeout(loadData,5000)}).catch(function(){showToast('Ошибка сети','error')})})}
...
function reconnectAll(srv){var label=_serverDisplayLabel(srv);uiConfirm('Переподключить отвалившиеся модемы на '+label+'?\n\nRe-Add всех офлайн-модемов сервера. Рабочие модемы не трогаются.',{title:'Переподключить модемы · '+label,okText:'Переподключить'}).then(function(ok){if(!ok)return;showToast('Переподключение модемов '+label+'…','info');api(API+'/api/admin/reconnect_all',{method:'POST',json:{serverName:srv}}).then(function(r){r.ok?showToast('Переподключено '+r.reconnected+'/'+r.total+' офлайн-модемов на '+label,'success',7000):showToast(r.error||'Ошибка','error');setTimeout(loadData,8000)}).catch(function(){showToast('Ошибка сети','error')})})}
...
function showDetails(btn){var nick=btn.dataset.nick,server=btn.dataset.server,map=currentData&&currentData._modemMap;if(!map)return;for(var imei in map){var m=map[imei];if(m.nick===nick&&m.server===server){currentDetailModem=m;break}}if(!currentDetailModem)return;document.getElementById('modalTitle').textContent=nick+' ('+_serverDisplayLabel(server)+')';switchTab('info',document.querySelector('.modal-tab[data-tab="info"]'));document.getElementById('detailModal').classList.add('show')}
function openDetailAtTab(nick,srv,tab){var map=currentData&&currentData._modemMap;if(!map)return;currentDetailModem=null;for(var imei in map){var m=map[imei];if(m.nick===nick&&m.server===srv){currentDetailModem=m;break}}if(!currentDetailModem)return;document.getElementById('modalTitle').textContent=nick+' ('+_serverDisplayLabel(srv)+')';var tabEl=document.querySelector('.modal-tab[data-tab="'+tab+'"]');switchTab(tab,tabEl);document.getElementById('detailModal').classList.add('show')}
...
function _renderOverview(body,m){
  ...
  if(att){var _d=(att.sev||3)>=4;bCol=_d?'var(--danger)':'var(--warning)';bBg=_d?'rgba(255,56,60,0.08)':'rgba(255,204,0,0.10)';bBd=_d?'rgba(255,56,60,0.25)':'rgba(255,204,0,0.28)';bIcon=_d?icon('stop',15):icon('alert',15);bTxt=att.reason;}
  else if(st==='rotating'){bCol='var(--accent)';bBg='var(--accent-dim)';bBd='var(--border)';bIcon='↻';bTxt='Смена IP…';}
  else if(st==='rebooting'){bCol='var(--accent)';bBg='var(--accent-dim)';bBd='var(--border)';bIcon=icon('power',15);bTxt='Перезагрузка…';}
  else {bCol='var(--success)';bBg='rgba(52,199,89,0.09)';bBd='rgba(52,199,89,0.25)';bIcon=icon('check',15);bTxt='Онлайн — активных проблем нет';}
  ...
  clients.forEach(function(c){chips+=_ovChip(icon('user',11)+' '+esc(c),'var(--accent)','var(--accent-dim)');});
  ...
  if(st!=='offline'){acts+=_ovActBtn('<span style="color:var(--accent)">↻</span> Сбросить IP',d,'resetIp(this)');acts+=_ovActBtn('<span style="color:var(--warning)">'+icon('power')+'</span> Ребут',d,'rebootModem(this)');}
  acts+=_ovActBtn('<span style="color:var(--text-2)">'+icon('refresh')+'</span> Re-Add',d,'readdModem(this)');
  ...
  var _up=m.uptimePct;var kUp=_ovKpiCard('Аптайм 30д',_up!=null?_up+'%':'—',(m.uptimeTotalChecks!=null)?(m.uptimeOnlineChecks+'/'+m.uptimeTotalChecks+' проверок'):'нет данных',_up!=null?(parseFloat(_up)>=99?'var(--success)':parseFloat(_up)>=95?'var(--warning)':'var(--danger)'):'var(--text-3)');
  var _ep=m.pcErrorPctToday;var kErr=_ovKpiCard('Сбои пинга',_ep!=null?_ep+'%':'—',(m.pcChecksToday!=null)?((m.pcErrToday||0)+' / '+m.pcChecksToday+' замеров сегодня'):'нет проверок',_ep!=null?(_ep>=_errorRateThreshold?'var(--danger)':_ep>0?'var(--warning)':'var(--success)'):'var(--text-3)');
  var kpis='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(140px,100%),1fr));gap:10px;margin-bottom:12px">'+kUp+kErr+'</div>';
  ...
  net+=_ovRow('IP',m.extIp?('<span style="cursor:pointer" data-on-click="copyText(\''+esc(m.extIp)+'\',this)">'+esc(m.extIp)+' '+icon('copy',11)+'</span>'+(m.ipStuck?' <span style="color:var(--warning)" title="IP не менялся '+m.ipSinceHours+'ч">'+icon('alert',10)+'</span>':'')):'—');
  net+=_ovRow('Пинг / потери',(function(){var _pg=(currentData.modemPing||{})[m.server+'_'+m.nick];if(_pg){var c=!_pg.ok?'var(--danger)':(!_pg.fresh?'var(--text-3)':(_pg.loss_pct>=30||_pg.latency_ms>800?'var(--warning)':'var(--success)'));return '<span style="color:'+c+';font-family:var(--font-mono)">'+_pg.latency_ms+' мс · '+_pg.loss_pct+'%</span>'+(_pg.ok?'':' <b style="color:var(--danger)">интернета нет</b>')+(_pg.fresh?'':' <span style="color:var(--text-3)">(данные протухли)</span>');}return (m.ping?esc(String(m.ping)):'—')+(m.pktLoss!=null?(' · '+m.pktLoss+'%'):'');})());
  net+='<div id="ovPingSpark" style="padding:2px 0 4px"></div>';
  net+=_ovRow('Сейчас',(function(){var _rt=(currentData.modemRate||{})[m.server+'_'+m.nick];if(!_rt)return '\u2014';var _tot=_rt.rate_in_mbps+_rt.rate_out_mbps;var _rc=_tot>=20?'var(--danger)':(_tot>=5?'var(--warning)':'var(--text-0)');return '<span style="color:'+_rc+';font-family:var(--font-mono)">↓ '+_rt.rate_in_mbps+' / ↑ '+_rt.rate_out_mbps+' Мбит/с</span>';})());
  net+='<div id="ovRateSpark" style="padding:2px 0 4px"></div>';
  net+=_ovRow('HTTP-чек',(function(){var _hc=(currentData.modemHttpCheck||{})[m.server+'_'+m.nick];if(!_hc){var _e=_httpEligibility(m);return '<span style="color:var(--text-3);font-size:10px">'+esc(_e.text)+'</span>';}var _skip=_hc.error==='offline'||_hc.error==='no_valid_client_credentials';var _ok=!_hc.error;var _dot=_skip?'var(--text-3)':(_ok?'var(--success)':'var(--danger)');var _txt=_hc.error==='offline'?'модем офлайн':_hc.error==='no_valid_client_credentials'?'нет действующего клиентского HTTP-порта':(_ok?(('HTTP '+_hc.status)+(_hc.total_ms!=null?' · '+_hc.total_ms+' мс':'')):('ошибка: '+_hc.error));return '<span style="display:inline-flex;align-items:center;gap:5px"><span style="width:8px;height:8px;border-radius:50%;background:'+_dot+';display:inline-block"></span><span style="font-size:11px;color:'+(_skip?'var(--text-3)':(_ok?'var(--text-0)':'var(--danger)'))+'">'+esc(_txt)+'</span></span>';})());
  ...
  net+=_ovRow('ICCID',m.iccid?('<span style="font-family:var(--font-mono);font-size:10px;cursor:pointer" data-on-click="copyText(\''+esc(m.iccid)+'\',this)">'+esc(m.iccid)+' '+icon('copy',11)+'</span>'):'—');
  net+=_ovRow('Обновление контракта',m.contractRenewalDate?esc(new Date(m.contractRenewalDate+'T00:00:00').toLocaleDateString('ru-RU')):'—');
  ...
  _ports.forEach(function(port,pi){
    ...
    if(_ports.length>1||port.portName){
      ...
      conn+='<div style="font-size:10px;font-weight:600;color:var(--accent);margin:2px 0 4px">'+esc(port.portName||('Порт '+(pi+1)))
        +(_pcred?' <span style="font-weight:400;cursor:pointer" title="ip:port:login:pass" data-on-click="copyText(\''+_pcred.replace(/'/g,"\\'")+'\',this)">'+icon('copy',11)+' Реквизиты</span>':'')
        +(_pcredLink?' <span style="font-weight:400;cursor:pointer" title="Реквизиты + ссылка для смены IP" data-on-click="copyText(\''+_pcredLink.replace(/'/g,"\\'")+'\',this)">'+icon('copy',11)+' Реквизиты+ссылка</span>':'')
        +'</div>';
    }
    ...
    var _pid='ovPwd_'+String(port.portID||pi).replace(/[^\w-]/g,'_');
    var _peeked=(window._peekRevealed&&window._peekRevealed[_pid])||null;
    conn+='<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 0"><span style="font-size:11px;color:var(--text-2)">Пароль</span><span style="font-size:10px;color:var(--text-0);font-family:var(--font-mono)"><span id="'+_pid+'">'+(port.LOGIN?(_peeked?esc(_peeked):'••••••••'):'—')+'</span>'+(port.LOGIN?' <span style="cursor:pointer;color:var(--text-2)" data-on-click="peekField(this,\''+_pid+'\',\''+esc(port.PASSWORD||'').replace(/'/g,"\\'")+'\')">'+icon(_peeked?'eyeOff':'eye',11)+'</span> <span style="cursor:pointer;color:var(--text-2)" data-on-click="copyText(\''+esc(port.PASSWORD||'').replace(/'/g,"\\'")+'\',this)">'+icon('copy',11)+'</span>':'')+'</span></div>';
  ...
  traf+='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:10px"><span style="font-size:11px;color:var(--text-2);text-transform:uppercase;letter-spacing:.5px;font-weight:600">Трафик</span><span style="font-size:10px;color:var(--accent);cursor:pointer" data-on-click="document.querySelector(\'.modal-tab[data-tab=&quot;traffic&quot;]\').click()">тепловая карта →</span></div>';
...
// A3 (23.08): спарклайн текущей скорости за сутки (снапшоты modem_rate, 5 мин).
function _loadRateSpark(m){
  var el=document.getElementById('ovRateSpark');if(!el||!m)return;
  api(API+'/api/admin/modem_rate?server='+encodeURIComponent(m.server)+'&nick='+encodeURIComponent(m.nick)+'&hours=24').then(function(d){
    var el2=document.getElementById('ovRateSpark');if(!el2)return;
    var rows=(d&&d.rows)||[];
    if(rows.length<2){el2.innerHTML='';return;}
    var W=240,H=26,vs=rows.map(function(r){return r.rate_in_mbps+r.rate_out_mbps});
    var mx=Math.max.apply(null,vs.concat([0.1]));
    var pts=rows.map(function(r,i){return (i/(rows.length-1)*W).toFixed(1)+','+(H-2-((r.rate_in_mbps+r.rate_out_mbps)/mx)*(H-4)).toFixed(1)}).join(' ');
    el2.innerHTML='<svg width="100%" height="'+H+'" viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none" style="display:block"><polyline points="'+pts+'" fill="none" stroke="var(--accent)" stroke-width="1.2"></polyline></svg><div style="font-size:9px;color:var(--text-3);margin-top:1px">трафик за 24ч · пик '+mx.toFixed(1)+' Мбит/с (in+out)</div>';
  }).catch(function(){var el3=document.getElementById('ovRateSpark');if(el3)el3.innerHTML='';});
}

// A1 (23.08): спарклайн пинга за сутки в карточке (история из modem_ping).
function _loadPingSpark(m){
  var el=document.getElementById('ovPingSpark');if(!el||!m)return;
  api(API+'/api/admin/modem_ping?server='+encodeURIComponent(m.server)+'&nick='+encodeURIComponent(m.nick)+'&hours=24').then(function(d){
    var el2=document.getElementById('ovPingSpark');if(!el2)return;
    var rows=(d&&d.rows)||[];
    if(rows.length<2){el2.innerHTML='';return;}
    var W=240,H=26,vs=rows.map(function(r){return r.latency_ms||0});
    var mx=Math.max.apply(null,vs.concat([1]));
    var pts=rows.map(function(r,i){return (i/(rows.length-1)*W).toFixed(1)+','+(H-2-((r.latency_ms||0)/mx)*(H-4)).toFixed(1)}).join(' ');
    var bad='';rows.forEach(function(r,i){if(!r.ok){var x=(i/(rows.length-1)*W).toFixed(1);bad+='<line x1="'+x+'" y1="0" x2="'+x+'" y2="'+H+'" stroke="var(--danger)" stroke-width="1" opacity=".55"/>'}});
    el2.innerHTML='<svg width="100%" height="'+H+'" viewBox="0 0 '+W+' '+H+'" preserveAspectRatio="none" style="display:block">'+bad+'<polyline points="'+pts+'" fill="none" stroke="var(--accent)" stroke-width="1.2"><title>Пинг за 24ч, макс '+mx+' мс</title></polyline></svg><div style="font-size:9px;color:var(--text-3);margin-top:1px">пинг за 24ч · макс '+mx+' мс · красные отметки — потери 100%</div>';
  }).catch(function(){var el3=document.getElementById('ovPingSpark');if(el3)el3.innerHTML='';});
}

function _renderSpeed(el,m){var sh='<div style="margin-bottom:10px;text-align:center"><button class="btn btn-primary" data-on-click="runSpeedtest(\''+esc(m.nick)+'\',\''+m.server+'\',\''+m.rawImei+'\')">Запустить Speedtest (~1.5 мин)</button></div>';
...
function _renderRotationLog(el,m){
  ...
  api(API+'/api/admin/rotation_log?nick='+encodeURIComponent(m.nick)+'&serverName='+m.server).then(function(data){
    ...
    entries.forEach(function(e){
      ...
      h+='<td style="font-family:var(--font-mono);color:'+(unchanged?'var(--warning,#c60)':'var(--accent)')+'">'+esc(String(newIp))+(unchanged?' <span title="IP не сменился" style="font-size:10px;color:var(--warning,#c60)">'+icon('alert',10)+' не сменился</span>':'')+'</td>';
      h+='</tr>';
    });
...
function renderTabContent(tab){var body=document.getElementById('modalBody'),m=currentDetailModem;if(!m)return;
  ...
  if(false){}
    ...
    var portsH='<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px">'
      ...
      +'<button class="btn btn-primary btn-sm" data-on-click="showNewPortForm(\''+m.rawImei+'\',\''+m.server+'\')">+ Добавить</button>'
      +'</div>';
    if(!m.ports.length){portsH+='<div style="color:var(--text-3);font-size:12px">Нет портов</div>'}
    else{m.ports.forEach(function(p){var pid=(p.portID||'').replace(/^S\d+_/,'');var osTag=p.OS?'<span style="display:inline-block;padding:1px 6px;border-radius:8px;font-size:9px;font-weight:600;background:rgba(25,96,201,.1);color:var(--accent);margin-left:6px">'+icon('shield',9)+' '+esc(p.OS)+'</span>':'';
      ...
      var statusTags='';
      if(expired)statusTags+='<span style="display:inline-block;padding:1px 6px;border-radius:8px;font-size:9px;font-weight:600;background:rgba(232,65,65,.15);color:var(--danger);margin-left:6px">'+icon('clock',9)+' ИСТЁК</span>';
      if(overQuota)statusTags+='<span style="display:inline-block;padding:1px 6px;border-radius:8px;font-size:9px;font-weight:600;background:rgba(239,159,39,.15);color:var(--warning);margin-left:6px">'+icon('box',9)+' ЛИМИТ</span>';
      var _bHold=_portBlockedInfo(m.server,pid);
      var _bc=_clientBlockedByPortName((p.portName||'').trim());
      if(_bHold!==null)statusTags+='<span style="display:inline-block;padding:1px 6px;border-radius:8px;font-size:9px;font-weight:600;background:rgba(232,65,65,.15);color:var(--danger);margin-left:6px" title="Прокси не работает, порт будет удалён'+(_bHold?' '+esc(_fmtIsoDt(_bHold)):'')+'">'+icon('x',9)+' ЗАБЛОКИРОВАН'+(_bHold?' · удалится '+esc(_fmtIsoDt(_bHold)):'')+'</span>';
      else if(_bc)statusTags+='<span style="display:inline-block;padding:1px 6px;border-radius:8px;font-size:9px;font-weight:600;background:rgba(232,65,65,.15);color:var(--danger);margin-left:6px" title="'+esc(_blockedClientTitle())+'">'+icon('x',9)+' ЗАБЛОКИРОВАН</span>';
      portsH+='<div style="display:flex;align-items:center;justify-content:space-between;padding:8px 10px;background:var(--bg-2);border-radius:8px;border:1px solid '+((_bHold!==null||_bc)?'var(--danger)':'var(--border)')+';margin-bottom:6px">'
      ...
      +'</div>'
      +'<div style="display:flex;gap:4px"><button class="btn btn-sm" data-on-click="movePortPrompt(\''+pid+'\',\''+m.server+'\',\''+esc(p.portName||pid)+'\')" title="Перекинуть на другой модем">↔</button>'
      +'<button class="btn btn-sm" data-on-click="editPortFull(\''+m.rawImei+'\',\''+m.server+'\',\''+pid+'\')" title="Настройки порта">'+icon('gear',12)+'</button>'
      +'<button class="btn btn-danger btn-sm" data-on-click="purgePort(\''+pid+'\',\''+m.server+'\')" title="Удалить">'+icon('x',12)+'</button></div>'
      +'</div>'});}
    ...
    body.innerHTML='<div style="display:flex;flex-direction:column;gap:16px;padding:4px 0">'
      ...
      +'<div style="display:grid;grid-template-columns:110px 1fr;align-items:center;gap:10px"><label style="font-size:12px;color:var(--text-2)">Обновление контракта</label><input class="form-input" type="date" id="msContractRenewal" value="'+esc(m.contractRenewalDate||'')+'"><span style="grid-column:2;font-size:10px;color:var(--text-3);margin-top:-7px">Дата следующего продления/обновления SIM-контракта</span></div>'
      ...
      +'<div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn btn-sm btn-warning" data-on-click="resetIpByImei(\''+m.rawImei+'\',\''+m.server+'\')" style="font-size:11px">'+icon('refresh',11)+' Сбросить IP сейчас</button>'
      +(/MF289/i.test(m.model||'')?'':'<button class="btn btn-sm" data-nick="'+esc(m.nick)+'" data-server="'+m.server+'" data-on-click="usbReset(this)" style="font-size:11px" title="Перезапустить USB-порт модема (usb reset)">'+icon('plug',11)+' USB Reset</button>')
      +'<button class="btn btn-sm" data-nick="'+esc(m.nick)+'" data-server="'+m.server+'" data-on-click="readdModem(this)" style="font-size:11px" title="Переподключить модем в системе ProxySmart (Re-Add device)">'+icon('plus',11)+' Re-Add</button>'
      +'</div>'
      +'</div></div>'
      // Section 3: Порты
      +'<div style="border:1px solid var(--border);border-radius:10px;overflow:hidden">'
      ...
      +'<button class="btn btn-primary" data-on-click="saveModemSettingsNew(\''+m.rawImei+'\',\''+m.server+'\',this)">Сохранить</button>'
      +'<button class="btn btn-sm" style="color:var(--text-2);font-size:11px" data-on-click="applyModemSettings(\''+m.rawImei+'\',\''+m.server+'\')" title="Сохранить и перезапустить модем">Сохранить и перезапустить</button>'
      +'</div>'
      +'</div>';
  }
}
function runSpeedtest(nick,srv,imei){
  ...
  function _stRender(d){
      var reloadBtn='<button class="btn btn-sm btn-primary" data-on-click="runSpeedtest(\''+esc(nick)+'\',\''+srv+'\',\''+esc(imei||'')+'\')">Повторить</button> ';
      if(d.error){
        body.innerHTML='<div style="padding:16px"><div style="color:var(--danger);font-size:12px;margin-bottom:8px">'+icon('alert',12)+' Ошибка: '+esc(d.error)+'</div>'+(d.details?'<div style="font-size:10px;color:var(--text-3);font-family:monospace;word-break:break-all">'+esc(d.details)+'</div>':'')+'<div style="margin-top:12px">'+reloadBtn+'</div></div>';
        return;
      }
      ...
      h+='<div style="margin-top:4px">'+reloadBtn+'<button class="btn btn-sm" data-on-click="loadSpeedHistory(\''+histKey+'\')">История</button></div>';
      // raw data
...
function showNewPortForm(imei,srv){var h='<div class="detail-card" style="margin-top:8px"><h4>Новый порт</h4><div class="form-group"><label>Имя порта</label><input class="form-input" id="npName" maxlength="64" placeholder="client_name" autocomplete="off"><div style="font-size:11px;color:var(--text-3);margin-top:5px">Латиница, цифры, точка, _ или -. ID, HTTP/SOCKS-порты и реквизиты назначит сам ProxySmart этого сервера.</div></div><button class="btn btn-primary" data-on-click="createPort(\''+imei+'\',\''+srv+'\')">Создать</button></div>';document.getElementById('newPortForm').innerHTML=h;setTimeout(function(){var input=document.getElementById('npName');if(input)input.focus()},0)}
function createPort(imei,srv){
  var pn=document.getElementById('npName').value.trim();
  if(pn.length<4){showToast('Имя порта должно быть не менее 4 символов','error');return}
  if(!/^[A-Za-z0-9._-]+$/.test(pn)){showToast('Только латиница, цифры, точка, _ и -','error');return}
...
function movePortPrompt(pid,srv,label){
  ...
  overlay.innerHTML='<div style="background:var(--bg-1);border-radius:12px;padding:20px;width:400px;max-width:90vw;box-shadow:0 24px 64px rgba(0,0,0,.5)" data-on-click="event.stopPropagation()">'
    ...
    +'<button class="btn btn-primary btn-sm" id="movePortBtn" data-on-click="doMovePort(\''+esc(pid)+'\',\''+esc(srv)+'\',this)">Перекинуть</button>'
    +'</div></div>';
...
function saveModemSettingsNew(imei,srv,btn){
  var rotSel=document.getElementById('msAutoRot');var rotRaw=rotSel?rotSel.value:'';
  var payload={serverName:srv,IMEI:imei,name:document.getElementById('msNick').value,PHONE_NUMBER:document.getElementById('msPhone').value,TARGET_MODE:document.getElementById('msMode').value,NOTES:document.getElementById('msNotes').value};
  if(rotRaw!=='__u')payload.AUTO_IP_ROTATION=String(parseInt(rotRaw)||0);
  var renewal=(document.getElementById('msContractRenewal')||{}).value||'';
  if(btn){btn.disabled=true;btn.textContent='Сохраняю…';}
  showToast('Сохраняю настройки на сервере — это может занять до ~20 сек (проверка после записи)…','info',20000);
  var _done=function(){if(btn){btn.disabled=false;btn.textContent='Сохранить';}};
  Promise.all([
    api(API+'/api/admin/store_modem',{method:'POST',json:payload}),
    api(API+'/api/admin/modems/'+encodeURIComponent(srv)+'/'+encodeURIComponent(imei)+'/contract',{method:'PUT',json:{renewal_date:renewal}})
  ]).then(function(rr){var r=rr[0],cr=rr[1];if(!cr.ok)throw new Error(cr.error||'Не удалось сохранить дату контракта');if(r.ok){showToast('Сохранено, применяю...','success');api(API+'/api/admin/apply_modem',{method:'POST',json:{imei:imei,serverName:srv}}).then(function(r2){_done();r2.ok?showToast('Применено','success'):showToast('Сохранено, но не применено: '+(r2.error||''),'warning');setTimeout(loadData,3000)}).catch(function(e){_done();showToast('Сохранено, но не применено: '+esc(e.message),'warning');setTimeout(loadData,3000)})}else{_done();showToast((r.error||'Ошибка')+(r.details?': '+esc(r.details):''),'error')}}).catch(function(e){_done();showToast('Ошибка: '+esc(e.message),'error')});
}
