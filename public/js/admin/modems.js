...
function loadHealthMap(){
  api(API+'/api/analytics/modem_health?days=7')
    .then(function(d){
      ...
      if (typeof renderTable === 'function') { try { renderTable(true); } catch(_){} }
    })
...
// (modem_health, heatmap), or null if included.
...
function _excludeReason(modem){
  ...
  if(!anyBound){
    return {
      type:'unbound',
      label:'Без клиента',
      tooltip:'Исключён из статистики: нет привязки к клиенту (ни один порт не имеет portName). Привяжите модем к клиенту, чтобы он попадал в здоровье и heatmap.'
    };
  }
  return null;
}
...
out+='<div class="modem-tile" data-nick="'+esc(m.nick)+'" data-server="'+m.server+'" data-imei="'+(m.rawImei||'')+'" data-model="'+esc(m.model||'')+'" data-stale="'+(_isStaleModem(m)?'1':'')+'" data-on-click="tileMenu(event,this)" style="border-left-color:'+c.b+';'+(c.bg?'background:'+c.bg+';':'')+'"><div class="mt-nick">'+esc(m.nick)+_modemFlags(m)+'</div><div class="mt-client" title="'+_clf+'">'+(_cls.length?_clf:'<span style="color:var(--text-3)">—</span>')+'</div>'+(m.operator?'<div class="mt-op" title="Оператор"><i></i>'+esc(m.operator)+'</div>':'')+'<div class="mt-sub">'+sub+'</div>'+_hs+_cConn+'</div>';});out+='</div>';});return out||'<div style="padding:40px;text-align:center;color:var(--text-3)">Нет модемов по фильтру</div>';}
  var _mkColor={webapp:'var(--danger)',sim:'var(--danger)',proxy:'var(--danger)',offline:'var(--danger)',loss:'var(--danger)',err:'var(--warning)',health:'var(--warning)',reboot:'var(--warning)'};
...
function renderTable(auto){
  ...
  COUNTRY_ORDER.forEach(function(srv){var modems=groups[srv];if(!modems||!modems.length)return;var ci=COUNTRIES[srv]||{};var online=0;modems.forEach(function(m){var s=getModemStatus(m);if(s==='online'||s==='rotating')online++});
    ...
    html+='<input type="checkbox" class="srv-bulk-chk" data-server="'+esc(srv)+'" data-on-change="bulkToggleServer(\''+esc(srv)+'\',this)" style="cursor:pointer;margin:0" title="Выбрать все модемы '+esc(srv)+'">';
    html+='<span style="font-size:14px">'+(ci.flag||'')+'</span>';
    html+='<span style="font-family:var(--font-mono);font-size:12px;font-weight:700;color:var(--accent)" title="Внутренний ID: '+esc(srv)+'">'+esc(ci.displayName||srv)+'</span>';
    ...
    // threshold the backend uses to drop modems from modem_health
...
h+='<span data-on-click="event.stopPropagation();'+_con+'" style="cursor:pointer;display:inline-flex;align-items:center;gap:6px" title="'+esc(_cparts.join('\n')||'нет активных подключений')+(_spark?'&#10;за час: макс '+_hmax:'')+'&#10;клик — лимиты порта">'+_spark+'<span class="mono" style="font-weight:'+(_ct>0?'600':'400')+';color:'+_cc+'">'+_ct+'</span></span>';break;}case'errors':{var ep=modem.pcErrorPct;if(ep!==null&&ep!==undefined){var eCls=ep>=_errorRateThreshold?'pc-bad':ep>0?'pc-warn':'pc-good';h+='<span class="pc-err '+eCls+'">'+ep+'%</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'health':{var _hm=_getHealth(modem);if(_hm&&_hm.health_score!=null){var _hs=_hm.health_score;var _hCls=_hm.status==='good'?'pc-good':_hm.status==='warn'?'pc-warn':'pc-bad';h+='<span class="pc-err '+_hCls+'" title="Сбои пинга: '+(_hm.error_pct||0)+'%, аптайм: '+(_hm.uptime_pct||0)+'% · нажмите «Инфо» для подробностей" style="cursor:help;font-weight:600">'+_hs+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'rotation':{var _rr=modem.autoRotation;if(_rr===''||_rr==null){h+='<span style="color:var(--text-3);font-size:10px" title="Нет данных с сервера">?</span>'}else{var rotMin=parseInt(_rr)||0;if(rotMin>0){var rotStr=rotMin>=60?(rotMin/60).toFixed(0)+'ч':rotMin+'м';h+='<span class="mono" style="font-size:11px">'+rotStr+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">Выкл</span>'}}break;}case'band':h+=modem.band?'<span class="mono" style="font-size:11px">'+esc(modem.band)+'</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'simStatus':{var _ss=(modem.simStatus||'');if(!_ss||_ss==='UNKNOWN'){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _ok=/\bOK\b|READY/.test(_ss);h+='<span class="pc-err '+(_ok?'pc-good':'pc-bad')+'" style="font-size:10px" title="'+esc(_ss)+'">'+esc(_ss)+'</span>'}break;}case'rebootScore':{var _rs=modem.rebootScore;if(_rs==null){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _rc=_rs>=70?'pc-bad':_rs>=50?'pc-warn':'pc-good';h+='<span class="pc-err '+_rc+'" style="font-size:10px">'+_rs+'</span>'}break;}case'httpRedirect':h+=modem.httpRedirect?'<span class="pc-err pc-bad" style="font-size:10px" title="Оператор навязал редирект — SIM без денег/блок">редирект</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'isLocked':h+=modem.isLocked?'<span title="Модем занят операцией/ротацией">'+icon('lock',11)+'</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'actions':{var d='data-imei="'+modem.rawImei+'" data-server="'+modem.server+'" data-nick="'+esc(modem.nick)+'"';h+='<div class="actions-cell" style="display:flex;gap:4px;align-items:center;justify-content:flex-end">'+(status!=='offline'?'<button class="row-act" '+d+' title="Сбросить IP" data-on-click="resetIp(this)">↻</button><button class="row-act" '+d+' title="Ребут модема" data-on-click="rebootModem(this)">'+icon('power',12)+'</button>':'')+'<button class="btn-details" '+d+' data-on-click="showDetails(this)"><svg width="12" height="12" viewBox="0 0 13 13" fill="none"><rect x="1" y="1" width="11" height="11" rx="2" stroke="currentColor" stroke-width="1.1"/><line x1="4" y1="4.5" x2="9" y2="4.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="6.5" x2="9" y2="6.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="8.5" x2="7" y2="8.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/></svg>Инфо</button>';
...
function renderHealthTab(body, m){
  ...
  api(API + '/api/analytics/modem_health?days=7')
    .then(function(d){
      ...
      html += '<b>Как считается:</b> Базовый скор = 100. Вычитаются баллы за долю неуспешных пингов, затем учитываются длительность ротаций, аптайм и состояние SIM/ребута. Латентность в скоринге не участвует.';
      html += '</div>';
...
function _renderOverview(body,m){
  ...
  var kpis='<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(min(140px,100%),1fr));gap:10px;margin-bottom:12px">'+kHealth+kUp+kErr+'</div>';
...
function _renderHealthTab2(body,m){
  body.innerHTML='<div id="ovSecHealth"></div>';
