...
h+='<span data-on-click="event.stopPropagation();'+_con+'" style="cursor:pointer;display:inline-flex;align-items:center;gap:6px" title="'+esc(_cparts.join('\n')||'нет активных подключений')+(_spark?'&#10;за час: макс '+_hmax:'')+'&#10;клик — лимиты порта">'+_spark+'<span class="mono" style="font-weight:'+(_ct>0?'600':'400')+';color:'+_cc+'">'+_ct+'</span></span>';break;}case'errors':{var ep=modem.pcErrorPct;if(ep!==null&&ep!==undefined){var eCls=ep>=_errorRateThreshold?'pc-bad':ep>0?'pc-warn':'pc-good';h+='<span class="pc-err '+eCls+'">'+ep+'%</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'health':{var _hm=_getHealth(modem);if(_hm&&_hm.health_score!=null){var _hs=_hm.health_score;var _hCls=_hm.status==='good'?'pc-good':_hm.status==='warn'?'pc-warn':'pc-bad';h+='<span class="pc-err '+_hCls+'" title="P50 latency: '+(_hm.latency_ms||'?')+' мс, ошибки: '+(_hm.error_pct||0)+'%, аптайм: '+(_hm.uptime_pct||0)+'% · нажмите «Инфо» для подробностей" style="cursor:help;font-weight:600">'+_hs+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'rotation':{var _rr=modem.autoRotation;if(_rr===''||_rr==null){h+='<span style="color:var(--text-3);font-size:10px" title="Нет данных с сервера">?</span>'}else{var rotMin=parseInt(_rr)||0;if(rotMin>0){var rotStr=rotMin>=60?(rotMin/60).toFixed(0)+'ч':rotMin+'м';h+='<span class="mono" style="font-size:11px">'+rotStr+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">Выкл</span>'}}break;}case'band':h+=modem.band?'<span class="mono" style="font-size:11px">'+esc(modem.band)+'</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'simStatus':{var _ss=(modem.simStatus||'');if(!_ss||_ss==='UNKNOWN'){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _ok=/\bOK\b|READY/.test(_ss);h+='<span class="pc-err '+(_ok?'pc-good':'pc-bad')+'" style="font-size:10px" title="'+esc(_ss)+'">'+esc(_ss)+'</span>'}break;}case'rebootScore':{var _rs=modem.rebootScore;if(_rs==null){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _rc=_rs>=70?'pc-bad':_rs>=50?'pc-warn':'pc-good';h+='<span class="pc-err '+_rc+'" style="font-size:10px">'+_rs+'</span>'}break;}case'httpRedirect':h+=modem.httpRedirect?'<span class="pc-err pc-bad" style="font-size:10px" title="Оператор навязал редирект — SIM без денег/блок">редирект</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'isLocked':h+=modem.isLocked?'<span title="Модем занят операцией/ротацией">'+icon('lock',11)+'</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'actions':{var d='data-imei="'+modem.rawImei+'" data-server="'+modem.server+'" data-nick="'+esc(modem.nick)+'"';h+='<div class="actions-cell" style="display:flex;gap:4px;align-items:center;justify-content:flex-end">'+(status!=='offline'?'<button class="row-act" '+d+' title="Сбросить IP" data-on-click="resetIp(this)">↻</button><button class="row-act" '+d+' title="Ребут модема" data-on-click="rebootModem(this)">'+icon('power',12)+'</button>':'')+'<button class="btn-details" '+d+' data-on-click="showDetails(this)"><svg width="12" height="12" viewBox="0 0 13 13" fill="none"><rect x="1" y="1" width="11" height="11" rx="2" stroke="currentColor" stroke-width="1.1"/><line x1="4" y1="4.5" x2="9" y2="4.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="6.5" x2="9" y2="6.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="8.5" x2="7" y2="8.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/></svg>Инфо</button>';
...
function _renderOverview(body,m){
  ...
  net+=_ovRow('HTTP-чек',(function(){var _hc=(currentData.modemHttpCheck||{})[m.server+'_'+m.nick];if(!_hc)return '<span style="color:var(--text-3);font-size:10px">нет данных (вне scope)</span>';var _ok=!_hc.error;var _dot=_ok?'var(--success)':'var(--danger)';var _txt=_hc.offline?'модем оффлайн':(_ok?(('HTTP '+_hc.status)+(_hc.total_ms!=null?' · '+_hc.total_ms+' мс':'')):('ошибка: '+_hc.error));return '<span style="display:inline-flex;align-items:center;gap:5px"><span style="width:8px;height:8px;border-radius:50%;background:'+_dot+';display:inline-block"></span><span style="font-size:11px;color:'+(_ok?'var(--text-0)':'var(--danger)')+'">'+esc(_txt)+'</span></span>';})());
...
function _renderProxyLat(el,m){
  el.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px;padding:0 2px">'
    ...
    + '<button class="btn btn-sm btn-primary" id="infoProxyCheckBtn" data-on-click="runManualProxyCheck(\''+esc(m.nick)+'\',\''+m.server+'\')" style="font-size:11px;padding:5px 14px;white-space:nowrap">'+icon('play',11)+' Запустить замер</button>'
    + '</div>'
