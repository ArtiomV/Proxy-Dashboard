...
function tileMenu(e,tile){e.stopPropagation();var n=tile.dataset.nick,s=tile.dataset.server,i=tile.dataset.imei||'',mdl=tile.dataset.model||'';var d='data-imei="'+i+'" data-server="'+s+'" data-nick="'+esc(n)+'"';var m=document.getElementById('tileMenu');if(!m){m=document.createElement('div');m.id='tileMenu';m.className='tile-menu';document.body.appendChild(m);}
  m.innerHTML='<div class="tm-h">'+esc(n)+' <span style="color:var(--text-3);font-weight:400">'+esc(s)+'</span></div>'
    +'<button class="tm-item" '+d+' data-on-click="rebootModem(this);tmClose()">⏻ Ребут</button>'
    +(/MF289/i.test(mdl)?'':'<button class="tm-item" '+d+' data-on-click="usbReset(this);tmClose()">⊘ USB-ресет</button>')
    +'<button class="tm-item" '+d+' data-on-click="readdModem(this);tmClose()">⟳ Re-Add (переподключить)</button>'
    +'<div style="height:1px;background:var(--border);margin:4px 0"></div>'
    +'<button class="tm-item" data-on-click="tmClose();openDetailAtTab(this.dataset.nick,this.dataset.srv,\'info\')" data-nick="'+esc(n)+'" data-srv="'+s+'">ⓘ Подробнее</button>'+((i&&tile.dataset.stale==='1')?'<div style="height:1px;background:var(--border);margin:4px 0"></div><button class="tm-item" style="color:var(--danger)" data-on-click="tmClose();deleteModem(this.dataset.srv,this.dataset.pid,this.dataset.nick)" data-srv="'+s+'" data-pid="meta_'+i+'" data-nick="'+esc(n)+'">🗑 Удалить</button>':'');
...
function renderTable(){
  ...
  COUNTRY_ORDER.forEach(function(srv){var modems=groups[srv];if(!modems||!modems.length)return;var ci=COUNTRIES[srv]||{};var online=0;modems.forEach(function(m){var s=getModemStatus(m);if(s==='online'||s==='rotating')online++});
    ...
    html+='<div style="margin-left:auto;display:flex;gap:6px"><button class="srv-act" data-on-click="resetCompleteServer(\''+srv+'\')">↻ Сброс IP</button><button class="srv-act srv-act-danger" data-on-click="rebootServer(\''+srv+'\')">⏻ Ребут</button></div>';
    html+='</div></td></tr>';
...
h+='<span data-on-click="event.stopPropagation();'+_con+'" style="cursor:pointer;display:inline-flex;align-items:center;gap:6px" title="'+esc(_cparts.join('\n')||'нет активных подключений')+(_spark?'&#10;за час: макс '+_hmax:'')+'&#10;клик — лимиты порта">'+_spark+'<span class="mono" style="font-weight:'+(_ct>0?'600':'400')+';color:'+_cc+'">'+_ct+'</span></span>';break;}case'errors':{var ep=modem.pcErrorPct;if(ep!==null&&ep!==undefined){var eCls=ep>=_errorRateThreshold?'pc-bad':ep>0?'pc-warn':'pc-good';h+='<span class="pc-err '+eCls+'">'+ep+'%</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'health':{var _hm=_getHealth(modem);if(_hm&&_hm.health_score!=null){var _hs=_hm.health_score;var _hCls=_hm.status==='good'?'pc-good':_hm.status==='warn'?'pc-warn':'pc-bad';h+='<span class="pc-err '+_hCls+'" title="P50 latency: '+(_hm.latency_ms||'?')+' мс, ошибки: '+(_hm.error_pct||0)+'%, аптайм: '+(_hm.uptime_pct||0)+'% · нажмите «Инфо» для подробностей" style="cursor:help;font-weight:600">'+_hs+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">\u2014</span>'}break;}case'rotation':{var _rr=modem.autoRotation;if(_rr===''||_rr==null){h+='<span style="color:var(--text-3);font-size:10px" title="Нет данных с сервера">?</span>'}else{var rotMin=parseInt(_rr)||0;if(rotMin>0){var rotStr=rotMin>=60?(rotMin/60).toFixed(0)+'ч':rotMin+'м';h+='<span class="mono" style="font-size:11px">'+rotStr+'</span>'}else{h+='<span style="color:var(--text-3);font-size:10px">Выкл</span>'}}break;}case'band':h+=modem.band?'<span class="mono" style="font-size:11px">'+esc(modem.band)+'</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'simStatus':{var _ss=(modem.simStatus||'');if(!_ss||_ss==='UNKNOWN'){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _ok=/\bOK\b|READY/.test(_ss);h+='<span class="pc-err '+(_ok?'pc-good':'pc-bad')+'" style="font-size:10px" title="'+esc(_ss)+'">'+esc(_ss)+'</span>'}break;}case'rebootScore':{var _rs=modem.rebootScore;if(_rs==null){h+='<span style="color:var(--text-3);font-size:10px">—</span>'}else{var _rc=_rs>=70?'pc-bad':_rs>=50?'pc-warn':'pc-good';h+='<span class="pc-err '+_rc+'" style="font-size:10px">'+_rs+'</span>'}break;}case'httpRedirect':h+=modem.httpRedirect?'<span class="pc-err pc-bad" style="font-size:10px" title="Оператор навязал редирект — SIM без денег/блок">редирект</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'isLocked':h+=modem.isLocked?'<span title="Модем занят операцией/ротацией">🔒</span>':'<span style="color:var(--text-3);font-size:10px">—</span>';break;case'actions':{var d='data-imei="'+modem.rawImei+'" data-server="'+modem.server+'" data-nick="'+esc(modem.nick)+'"';h+='<div class="actions-cell" style="display:flex;gap:4px;align-items:center;justify-content:flex-end">'+(status!=='offline'?'<button class="row-act" '+d+' title="Сбросить IP" data-on-click="resetIp(this)">↻</button><button class="row-act" '+d+' title="Ребут модема" data-on-click="rebootModem(this)">⏻</button>':'')+'<button class="btn-details" '+d+' data-on-click="showDetails(this)"><svg width="12" height="12" viewBox="0 0 13 13" fill="none"><rect x="1" y="1" width="11" height="11" rx="2" stroke="currentColor" stroke-width="1.1"/><line x1="4" y1="4.5" x2="9" y2="4.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="6.5" x2="9" y2="6.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/><line x1="4" y1="8.5" x2="7" y2="8.5" stroke="currentColor" stroke-width="1" stroke-linecap="round"/></svg>Инфо</button>';
...
if(_pid){h+='<button class="btn-delete-modem" title="Удалить модем из дашборда" data-on-click="deleteModem(\''+modem.server+'\',\''+_pid+'\',\''+esc(modem.nick)+'\')">🗑</button>';}}
...
function _renderOverview(body,m){
  ...
  net+=_ovRow('IP',m.extIp?('<span style="cursor:pointer" data-on-click="copyText(\''+esc(m.extIp)+'\',this)">'+esc(m.extIp)+' 📋</span>'+(m.ipStuck?' <span style="color:var(--warning)" title="IP не менялся '+m.ipSinceHours+'ч">⚠</span>':'')):'—');
  ...
  net+=_ovRow('ICCID',m.iccid?('<span style="font-family:var(--font-mono);font-size:10px;cursor:pointer" data-on-click="copyText(\''+esc(m.iccid)+'\',this)">'+esc(m.iccid)+' 📋</span>'):'—');
...
function _renderProxyLat(el,m){
  el.innerHTML = '<div style="display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:12px;padding:0 2px">'
    ...
    + '<button class="btn btn-sm btn-primary" id="infoProxyCheckBtn" data-on-click="runManualProxyCheck(\''+esc(m.nick)+'\',\''+m.server+'\')" style="font-size:11px;padding:5px 14px;white-space:nowrap">▶ Запустить замер</button>'
    + '</div>'
...
function renderTabContent(tab){var body=document.getElementById('modalBody'),m=currentDetailModem;if(!m)return;
  ...
  if(false){}
    ...
    if(!m.ports.length){portsH+='<div style="color:var(--text-3);font-size:12px">Нет портов</div>'}
      ...
      +'<button class="btn btn-sm" data-on-click="editPortFull(\''+m.rawImei+'\',\''+m.server+'\',\''+pid+'\')" title="Настройки порта">⚙️</button>'
      +'<button class="btn btn-danger btn-sm" data-on-click="purgePort(\''+pid+'\',\''+m.server+'\')" title="Удалить">✕</button></div>'
      +'</div>'});}
    ...
    body.innerHTML='<div style="display:flex;flex-direction:column;gap:16px;padding:4px 0">'
      ...
      +'<div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn btn-sm btn-warning" data-on-click="resetIpByImei(\''+m.rawImei+'\',\''+m.server+'\')" style="font-size:11px">🔄 Сбросить IP сейчас</button>'
      +(/MF289/i.test(m.model||'')?'':'<button class="btn btn-sm" data-nick="'+esc(m.nick)+'" data-server="'+m.server+'" data-on-click="usbReset(this)" style="font-size:11px" title="Перезапустить USB-порт модема (usb reset)">🔌 USB Reset</button>')
      +'<button class="btn btn-sm" data-nick="'+esc(m.nick)+'" data-server="'+m.server+'" data-on-click="readdModem(this)" style="font-size:11px" title="Переподключить модем в системе ProxySmart (Re-Add device)">➕ Re-Add</button>'
      +'</div>'
      +'</div></div>'
      // Section 3: Порты
      +'<div style="border:1px solid var(--border);border-radius:10px;overflow:hidden">'
