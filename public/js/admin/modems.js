...
function _renderOverview(body,m){
  ...
  _ports.forEach(function(port,pi){
    ...
    var _pid='ovPwd'+pi;
    conn+='<div style="display:flex;justify-content:space-between;align-items:center;padding:3px 0"><span style="font-size:11px;color:var(--text-2)">Пароль</span><span style="font-size:10px;color:var(--text-0);font-family:var(--font-mono)"><span id="'+_pid+'">'+(port.LOGIN?'••••••••':'—')+'</span>'+(port.LOGIN?' <span style="cursor:pointer;color:var(--text-2)" data-on-click="peekField(this,\''+_pid+'\',\''+esc(port.PASSWORD||'').replace(/'/g,"\\'")+'\')">'+icon('eye',11)+'</span> <span style="cursor:pointer;color:var(--text-2)" data-on-click="copyText(\''+esc(port.PASSWORD||'').replace(/'/g,"\\'")+'\',this)">'+icon('copy',11)+'</span>':'')+'</span></div>';
