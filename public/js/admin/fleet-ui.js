...
function renderModemsTopBar(){
  ...
  // Отдельного «Оффлайн >12ч» больше нет (2026-07-28): давно молчащие модемы —
  // просто «Офлайн», как и короткие отключения.
  var offline=0,problem=0,free=0,sim=0;
  ...
  clSel+='</select>';
  var toggle='<div class="acc-period-group" style="display:inline-flex"><button class="acc-period-btn'+(window._modemsView==='grid'?'':' active')+'" data-on-click="setModemsView(\'table\')">Таблица</button><button class="acc-period-btn'+(window._modemsView==='grid'?' active':'')+'" data-on-click="setModemsView(\'grid\')">Сетка</button></div>';
