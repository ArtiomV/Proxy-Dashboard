...
- **bills** — счета: amount, status (unpaid/paid/replaced), bill_number (сквозной «N/MM-YYYY» — §2.5; до 17.08.2026 — «N/YYYY»), tochka_bill_id, formula (JSON-разбор расчёта; при ручной правке — edited_from). UNIQUE(client_id, period) — анти-дабл гейт (B2, миграция 056).
- **closing_documents** — акты: items (JSON позиций), total_amount, status (unsigned/signed), tochka_doc_id, act_number (сквозной «N/MM-YYYY» — §2.5; до 17.08.2026 — «N/YYYY»). UNIQUE(client_id, period, type) — анти-дабл гейт (B2, миграция 056). Позиции правятся из админки; в банке — только delete+create.
...
### 2.5. Акты
- **Нумерация актов и счетов (B2, Р15/Р23; обновлено 17.08.2026):** сквозная «№ N/MM-YYYY» — **единый счётчик для актов и счетов вместе внутри месяца ПЕРИОДА документа** (таблица `doc_numbering_monthly`: ym → next_num, миграция 068; до этого была годовая серия «N/YYYY» из `doc_numbering` — старые документы не перенумеровываются). Номер привязан к месяцу, к которому относится документ: акт за июль, выставленный в августе, получает номер июльской серии. Номер выдаётся атомарно (INSERT OR IGNORE + UPDATE … RETURNING в одной транзакции), старт с 1 для каждого нового месяца. Дыры от удалённых документов НЕ переиспользуются; удаление фиксируется в audit_log с номером (delete_closing_document/delete_bill). Документы старого формата «АКТ-YYYYMM-<id4>» не перенумеровываются. Анти-дабл: UNIQUE(client_id, period, type) на closing_documents и UNIQUE(client_id, period) на bills — истина; in-memory проверки в роутах/кроне — fast-path (create_act/create_bill отвечают 409 при существующем документе за период). Храним наш номер (act_number/bill_number) + tochka_doc_id/tochka_bill_id.
