...
- **bills** — счета: amount, status (unpaid/paid/replaced), bill_number («NN/MM-YYYY»: поклиентский NN + месяц периода — §2.5; ранее «N/MM-YYYY» и «N/YYYY»), tochka_bill_id, formula (JSON-разбор расчёта; при ручной правке — edited_from). UNIQUE(client_id, period) — анти-дабл гейт (B2, миграция 056).
- **closing_documents** — акты: items (JSON позиций), total_amount, status (unsigned/signed), tochka_doc_id, act_number («NN/MM-YYYY»: поклиентский NN + месяц периода — §2.5; ранее «N/MM-YYYY» и «N/YYYY»). UNIQUE(client_id, period, type) — анти-дабл гейт (B2, миграция 056). Позиции правятся из админки; в банке — только delete+create.
...
- **uptime_tracking / uptime_daily** — текущее состояние и минутные `online/total`-проверки модемов, включая пропажу модема или сервера; `last_online_check`, `offline_alerted` обеспечивают парность алертов «отключился/вернулся».
...
- **proxy_checks** — HTTP-проверки через прокси (status, error и время ответа).
- **client_uptime_daily** — те же минутные проверки, закреплённые за владельцем модема в момент опроса; **server_downtime**, **failover_log**, **auto_reboot_log**, **operator_country_map** (auto+manual).
...
- **monthly_costs** — ручные расходы per (period, category, subkey=сервер/оператор). Расходы на SIM рассчитываются из `app_settings.operator_packages`; старые строки `category=sim` используются только как временный fallback для незаполненного пакета.
...
### 2.5. Акты
- **Нумерация актов и счетов (B2, Р15/Р23; обновлено 17.08.2026, модель v2):** «№ NN/MM-YYYY» — **NN — поклиентский сквозной номер** (таблица `doc_numbering_client`: client_id → next_num, миграция 069; непрерывный с начала работы с клиентом, НЕ сбрасывается по месяцам; акты и счета делят одну серию клиента), **MM-YYYY — месяц ПЕРИОДА документа**. По номеру видна сквозная история клиента, по суффиксу — за какой месяц документ. Исторические серии: до 17.08 — годовая «N/YYYY» (`doc_numbering`), затем несколько часов помесячная системная «N/MM-YYYY» (`doc_numbering_monthly`, миграция 068) — старые документы не перенумеровываются. Номер выдаётся атомарно (INSERT OR IGNORE + UPDATE … RETURNING в одной транзакции). Дыры от удалённых документов НЕ переиспользуются; удаление фиксируется в audit_log с номером (delete_closing_document/delete_bill). Документы старого формата «АКТ-YYYYMM-<id4>» не перенумеровываются. Анти-дабл: UNIQUE(client_id, period, type) на closing_documents и UNIQUE(client_id, period) на bills — истина; in-memory проверки в роутах/кроне — fast-path (create_act/create_bill отвечают 409 при существующем документе за период). Храним наш номер (act_number/bill_number) + tochka_doc_id/tochka_bill_id.
...
- **Требует внимания** (4 карточки): «Проблемы инфраструктуры» (плитки Модем отключен / Низкая скорость / Завис IP / Сбоит прокси — клик открывает попап со списком); «Сбоит прокси» включает только активные клиентские прокси с подтверждённой проблемой: устойчивые потери или 3 ping-провала подряд, 2 HTTP-провала подряд, разрыв соединения, проблема SIM/редирект либо недоступный ProxySmart WebApp. Одиночный провал, офлайн, низкая скорость и зависший IP не дублируются. Остальные карточки: «Потребление трафика» (тренд 6 мес); «Выручка: факт и прогноз» (стек-бар «За ГБ»/«За модем» + run-rate прогноз месяца полупрозрачным столбцом, поповер «Формула» — см. §2.6); «Операторы» (трафик/модем/сут, ₽/ГБ себестоимость).
...
- **Инфраструктура** (раскрывашки): компактный блок «Ротации IP и ёмкость» (периоды 1–30 дн): три итога — ротации/успешность, уникальные IP/подсети, модемы/серверы/трафик — и две расшифровки «Надёжность ротаций» и «Разнообразие IP». Ниже — сверка биллинга. Отдельного блока «Топ проблемных модемов» нет — список объединён с плиткой «Сбоит прокси».
...
- Таблица: rail причины, статус-пилюля (+«блип»), флаги 🧪⛔📴♻🔒🌐⚠, чипы исключения «Не в стат.»/«Без клиента», portName-бейджи, креды (копия), IP; мониторинговые колонки идут **Пинг ProxySmart → HTTP → трафик**. Затем скорость, аптайм, TCP-подключения со спарклайном 60 мин и ротация. Отдельного столбца «Сбои пинга» нет: потери уже показаны в «Пинг» и используются внутри мониторинга. По серверу: «↻ Сброс IP», «⏻ Ребут» (пароль).
- Bulk: сброс IP, ребут, OS-spoof, ротация, экспорт (формат/фильтры/скачать), proxy-check, удаление.
- Карточка модема: Обзор (баннер состояния, действия Сброс IP/Ребут/Re-Add/Доступ, аптайм за 30 дней, Сеть и сигнал, Подключение — по каждому порту своя «📋 строка» креда, Трафик, тоггл тест-пула), Трафик (heatmap + топ доменов модема), История (ротации IP, спидтесты + запуск), Настройки (идентификация, сеть/ротация, порты: перенос/редактирование/удаление/добавление). SMS/USSD, VPN-профиль. Сводной оценки «Здоровье» нет.
...
- **SLA**: помесячная доступность по единым минутным `online/total`-проверкам. `uptime_daily` питает админку, `client_uptime_daily` фиксирует владельца при каждом опросе; для истории до внедрения владелец восстанавливается по `traffic_hourly`. API: `GET /api/client/sla_report?month=YYYY-MM`.
...
`POST /api/admin/run_billing` (sync опция), `POST /billing_rerun` (пересчёт за дату из durable, dry_run); `GET/POST /api/admin/monthly_costs` (в GET также `operator_costs`, `unconfigured_operators` и display-name серверов); `GET /api/admin/finance_dashboard` (выручка 30д факт + run-rate прогноз EOM, ARR/churn/ARPU/NRR/концентрация/utilization/RPM/затраты, включая автоматически рассчитанные пакеты операторов, тренд 12 мес/выручка по дням/платежи — формулы §2.6); `GET /api/admin/billing/reconciliation`.
...
### 6.6. Аналитика (admin)
`/api/analytics/monthly_traffic`, `/heatmap`, `/modem_heatmap`, `/rotations`, `/ip_stats`, `/traffic_forecast`, `/capacity`, `/latency_stats`, `/logs_domains_full`; `/api/admin/daily_traffic`, `/backfill_daily_traffic`, `/bandwidth_single|period`, `/reset_bandwidth`, `/unique_ips`, `/proxy_checks`, `POST /proxy_check`, `/top_hosts`, `/domain_guard` (+`/run`).
