...
-- Equipment accounting belongs to a physical location (servers sharing the
-- same normalized address share the same location_key).
...
CREATE INDEX IF NOT EXISTS idx_equipment_location ON equipment_inventory(location_key);

-- Imported ICCID -> phone directory. The poller uses it only when ProxySmart
-- has no phone, so a live value remains the highest-priority source.
